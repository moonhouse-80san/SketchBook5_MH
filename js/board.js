...
if(bd.find('div.rd').length){
	...
	bd.find('.to_sns a').click(function(){
		...
		if(!type){
			...
			loc = '//twitter.com/intent/tweet?text='+title+' '+url;
		} else if(type=="pinterest"){
			loc = '//pinterest.com/pin/create/bookmarklet/?url='+url+'&description='+title;
		} else if(type=="kakaostory"){
			loc = 'https://story.kakao.com/s/share?url='+encodeURIComponent(document.URL);
		} else if(type=="band"){
			var loc = '//band.us/plugin/share?body='+url+'&t='+title;
		} else if(type=="kakao"){
			if(img){
...
function reComment(doc_srl,cmt_srl,edit_url){
	...
	o.find('.reply_stk').attr('onclick', 'loadStickerList(undefined, '+cmt_srl+');'); // 스티커 대댓글
}
